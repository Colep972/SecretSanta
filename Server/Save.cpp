...
bool Save::saveDrawResult(const std::string& crewFile,
...
bool Save::loadDrawResult(const std::string& crewFile,
    std::map<std::string, std::string>& results)
{
    ...
    if (!in.is_open()) return false;
    json j; in >> j;
    if (!j.contains("draw")) return false;
...
bool Save::saveProfile(const Profile& profile, const std::string& filename)
{
    ...
    j["password_hash"] = profile.getPasswordHash();
    j["name"] = profile.getName();
    j["email"] = profile.getEmail();
    j["wishes"] = profile.getWishes();
    j["crews"] = json::array();
...
bool Save::loadProfile(Profile& profile, const std::string& filename)
{
    ...
    profile = Profile(
        ...
        j["password_hash"].get<std::string>(),
        j["name"].get<std::string>(),
        j["email"].get<std::string>()
    );

    for (const auto& w : j["wishes"])
        profile.addWish(w.get<std::string>());

    for (const auto& jc : j["crews"])
        profile.addCrew(
            jc["code"].get<std::string>(),
            jc["token"].get<std::string>()
        );
